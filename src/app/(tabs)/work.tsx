...
export default function WorkScreen() {
  ...
  return (
    <AppScreen>
      ...
      {age >= 14 ? <MenuTile icon="💼" title="Jobs" subtitle="Browse openings, build a reputation, train, network, navigate office politics, and turn a job into an actual career." route="/jobs" badge={career ? career.title : 'Looking'} /> : null}
      {age >= 15 ? <MenuTile icon="🎓" title="College & training" subtitle="Choose a direction, build a network, intern, compete in sports, pay tuition, or walk away." route="/college" badge={education && education.status !== 'school' ? education.status : 'Options'} /> : null}
      {age >= 16 ? <MenuTile icon="🏢" title="Businesses" subtitle="Start companies, shape strategy, invest in quality and R&D, hire leadership, expand, raise money, and build an empire." route="/businesses" badge={`${businesses.length} owned`} /> : null}
      ...
      {privateMovement ? <MenuTile icon="◈" title="Inner Circle" subtitle="A private movement with followers, resources, doctrine, internal politics, and consequences outside the normal menus." route="/inner-circle" badge={`${privateMovement.followers.toLocaleString()} following`} /> : null}
