...
export default function WorkScreen() {
  ...
  return (
    <AppScreen>
      <View style={styles.header}><View style={{ alignSelf: 'stretch', gap: 4 }}><Eyebrow>CAPABILITY & POWER</Eyebrow><Heading size="large">Work</Heading><Body secondary>School, career, business, organizations, and public office share one world.</Body></View><StatusPill tone="accent">Knowledge {Math.round(actor.knowledge)}</StatusPill></View>

      <View style={styles.section}>
        <SectionHeader title="Education" />
        {education.length === 0 ? (
          <Card><Body secondary>No formal record yet. Education begins with age and context; competence can also grow outside credentials.</Body><EngineActionButton title="Prioritize learning" action={{ verb: 'education.study', targetIds: [], parameters: {} }} tone="accent" /></Card>
      ...
      <View style={styles.section}>
        ...
        <Card>
          <View style={styles.row}><View style={{ flex: 1, gap: 3 }}><Heading size="small">{politics?.office ?? politics?.campaign?.office ?? 'Private citizen'}</Heading><Body secondary>{politics?.campaign ? `${politics.campaign.weeksRemaining} weeks to election` : `${memberships.length} memberships · ${politics?.authority ?? 0} authority`}</Body></View>{politics?.campaign ? <StatusPill tone="warning">Campaigning</StatusPill> : politics?.office ? <StatusPill tone="success">In office</StatusPill> : <StatusPill>Eligible path</StatusPill>}</View>
      ...
      <OtherActionComposer domains={['education', 'career', 'business', 'organization', 'politics']} placeholder="Start a company, apply for a role, raise capital, or join an organization…" />
    </AppScreen>
  );
}
