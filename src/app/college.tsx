...
export default function CollegeScreen() {
  ...
  if (sportsOpen) {
    return (
      <AppScreen>
        <SubviewHeader eyebrow="College" title="Athletics" subtitle="This can be a social activity—or an entire parallel career track with skill, training, competition, recruiting, scholarships, fame, injury risk, and eventually professional interest." />
    ...
    <AppScreen>
      ...
      {current ? (
        <Card accent>
          ...
          <View style={styles.stats}><Stat label="Grades" value={Math.round(current.recordedGrade).toString()} /><Stat label="Academic skill" value={Math.round(academicSkill).toString()} /><Stat label="Network" value={Math.round(current.network).toString()} /><Stat label="Tuition / yr" value={formatMoney(annualTuition, true)} /></View>
