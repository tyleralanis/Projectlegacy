...
export interface EconomyState { regime: 'recession' | 'slow' | 'steady' | 'growth' | 'boom'; growth: number; inflation: number; policyRate: number; housingIndex: number; marketIndex: number; unemployment: number; }
...
export interface Security { id: EntityId; symbol: string; name: string; sector: string; priceCents: MoneyCents; quality: number; volatility: number; dividendYieldBps: number; }
export interface Holding { id: EntityId; ownerId: EntityId; securityId: EntityId; unitsMilli: number; costBasisCents: MoneyCents; }
export interface Liability { id: EntityId; debtorId: EntityId; kind: 'mortgage' | 'business' | 'student' | 'credit' | 'legal'; principalCents: MoneyCents; annualRateBps: number; weeklyPaymentCents: MoneyCents; securedById?: EntityId; }
export interface PoliticalState { characterId: EntityId; partyId?: EntityId; office?: string; officeLevel?: 'local' | 'regional' | 'national'; authority: number; approval: number; campaign?: { office: string; weeksRemaining: number; fundsCents: MoneyCents; support: number; opposition: number; }; }
export interface LegalExposure { id: EntityId; characterId: EntityId; category: string; severity: number; evidence: number; discoverability: number; createdWeek: number; discovered: boolean; resolved: boolean; }
export interface LegalCase { id: EntityId; characterId: EntityId; exposureId: EntityId; stage: 'investigation' | 'charged' | 'trial' | 'appeal' | 'resolved'; counselQuality: number; risk: number; outcome?: 'dismissed' | 'settled' | 'acquitted' | 'convicted'; }
export interface TransactionRecord { id: EntityId; week: number; kind: string; amountCents: MoneyCents; fromId?: EntityId; toId?: EntityId; memo: string; }
export interface EventChoice { id: string; label: string; detail: string; tone?: 'default' | 'positive' | 'danger'; }
export interface GameEvent { id: EntityId; templateId: string; domain: Domain; severity: Severity; week: number; title: string; narrative: string; participantIds: EntityId[]; choices: EventChoice[]; otherActionFamilies: string[]; resolved: boolean; selectedChoiceId?: string; explanation?: OutcomeExplanation; }
export interface FeedEntry { id: EntityId; week: number; domain: Domain; title: string; detail: string; important: boolean; }
export type TimelineCategory = 'birth' | 'relationship' | 'education' | 'career' | 'business' | 'property' | 'wealth' | 'politics' | 'legal' | 'world' | 'death' | 'dynasty';
export interface OutcomeFactor { label: string; impact: 'positive' | 'negative' | 'neutral'; detail: string; }
export interface OutcomeExplanation { summary: string; factors: OutcomeFactor[]; }
export interface TimelineEntry { id: EntityId; week: number; generation: number; category: TimelineCategory; title: string; detail: string; subjectIds: EntityId[]; importance: 1 | 2 | 3 | 4 | 5; explanation?: OutcomeExplanation; }
export type FavoriteEntityType = 'character' | 'business' | 'property' | 'organization' | 'country' | 'event';
export interface FavoriteRef { id: EntityId; entityType: FavoriteEntityType; entityId: EntityId; label: string; pinnedAtWeek: number; }
export interface IntentAuditEntry { id: EntityId; week: number; input: string; mode: 'baseline' | 'foundation_models' | 'unavailable'; confidence: number; status: 'executed' | 'confirmed' | 'clarified' | 'fallback' | 'rejected' | 'error'; proposedVerb?: string; targetIds: EntityId[]; diagnostics: string[]; }
export interface CountryState { id: EntityId; name: string; population: number; stability: number; ruleOfLaw: number; educationIndex: number; marketAccess: number; }
...
export interface AdvanceResult { world: WorldState; summary: AdvanceSummary; }
export interface IntentAction { verb: string; targetIds: EntityId[]; parameters: Record<string, string | number | boolean | null>; destructive?: boolean; }
export interface ActionValidation { valid: boolean; reason?: string; prerequisites?: string[]; requiresConfirmation: boolean; }
export interface ActionResult { world: WorldState; validation: ActionValidation; message: string; explanation?: OutcomeExplanation; }
